...
function Fail([string]$m){
  throw ("HASHCANON_NFL_PACKET_SELFTEST_FAIL:" + $m)
}
...
$PacketParent = Join-Path $RepoRoot "test_vectors\hashcanon_optionA\minimal_packet\packet"
if(-not (Test-Path -LiteralPath $PacketParent -PathType Container)){
  Fail ("PACKET_PARENT_MISSING:" + $PacketParent)
}

$Dirs = @(
  @(Get-ChildItem -LiteralPath $PacketParent -Directory -ErrorAction Stop | Sort-Object Name -Descending)
)

if($Dirs.Count -lt 1){
  Fail "NO_PACKET_DIRS_FOUND"
}
...
if([string]::IsNullOrWhiteSpace($d1)){
  Fail "RUN1_MISSING_DIGEST"
}

if([string]::IsNullOrWhiteSpace($d2)){
  Fail "RUN2_MISSING_DIGEST"
}
